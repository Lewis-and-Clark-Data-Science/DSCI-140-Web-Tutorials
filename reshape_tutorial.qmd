...
climate_data_longer <- climate_data |>
...
Here are a few exercises to try. You can click for hints and the answer for each problem.
...
```{webr-r}
# Your code here.  If run successfully, the following code would indicate that your data has
# 16 observations and 5 variables

str(climate_data_longer)
```
...
# climate_data_wider <- climate_data_longer |> 
...
str(climate_data_reversion) # should show 32 observations, 5 variables
```
...
str(all_climate_data) # should now have 20 observations
```
...
str(climate_treaty_info) # If run successfully you sould have more than 5 variables
```
...
```{webr-r}
# Here's how you start:
# climate_treaty_info <- left_join(____, ______, by = "______")
```
:::
...
str(missing_countries) # should have 1 observation, Elbonia
```
